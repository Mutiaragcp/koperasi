...
<div class="content-header d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
    <div>
        <h5 class="fw-bold text-dark mb-0">Registrasi Anggota Baru</h5>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 small mt-1">
                ...
                <li class="breadcrumb-item active text-dark fw-medium" aria-current="page">Tambah Anggota</li>
            </ol>
        </nav>
        <p class="text-muted-light small mt-2 mb-0">Input data anggota baru ke dalam database koperasi.</p>
    </div>
...
<form action="/anggota" method="POST" enctype="multipart/form-data">
    ...
    <div class="row g-4">
            ...
            <div class="card card-modern border-0 shadow-sm p-4 h-100">
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <input type="text" name="nama" class="form-control bg-light border-0 py-2 @error('nama') is-invalid @enderror" value="{{ old('nama') }}" placeholder="Contoh: Andi Wijaya" required>
                        @error('nama') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-12">
                        <label class="form-label fw-semibold small">Email (Username Login) <span class="text-danger">*</span></label>
                        <input type="email" name="email" class="form-control bg-light border-0 py-2 @error('email') is-invalid @enderror" value="{{ old('email') }}" placeholder="Contoh: andi@email.com" required>
                        @error('email') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-12">
                        <label class="form-label fw-semibold small">Password Default</label>
                        <input type="text" class="form-control bg-light border-0 py-2 text-muted" value="password123" readonly title="Password akan diatur otomatis menjadi: password123">
                        <span class="d-block mt-2 small text-muted-light">Anggota dapat mengubah sandi ini nanti setelah login.</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card card-modern border-0 shadow-sm p-4 h-100">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nik" class="form-control bg-light border-0 py-2 @error('nik') is-invalid @enderror" value="{{ old('nik') }}" placeholder="3201xxxxxxxxx" required>
                        @error('nik') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold small">Nomor WhatsApp <span class="text-danger">*</span></label>
                        <input type="text" name="no_hp" class="form-control bg-light border-0 py-2 @error('no_hp') is-invalid @enderror" value="{{ old('no_hp') }}" placeholder="0812xxxx" required>
                        @error('no_hp') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold small">Tanggal Gabung</label>
                        <input type="date" class="form-control bg-light border-0 py-2 text-muted" value="{{ date('Y-m-d') }}" readonly>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold small">Status</label>
                        <input type="text" class="form-control bg-light border-0 py-2 text-success fw-medium" value="Aktif" readonly>
                    </div>
                    <div class="col-12">
                        <label class="form-label fw-semibold small">Upload Foto KTP (Opsional)</label>
        ...
        <div class="col-12">
            <div class="card card-modern border-0 shadow-sm p-4">
                ...
                <textarea name="alamat" class="form-control bg-light border-0 py-2 @error('alamat') is-invalid @enderror" rows="2" placeholder="Jl. Raya Koperasi, No. 10..." required>{{ old('alamat') }}</textarea>
    ...
    <div class="d-flex justify-content-end mt-4 gap-2">
        ...
        <button type="submit" class="btn btn-primary px-5 py-2 fw-bold shadow">Simpan Anggota Baru</button>
    </div>
</form>
@endsection